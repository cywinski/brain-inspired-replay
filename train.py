...
def train_cl(
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        active_classes = (
            None  # -> for "domain"- or "all"-scenarios, always all classes are active
        )
        if scenario == "task":
            ...
            if Generative:
                ...
                conditional_gen = (
                    ...
                    if (
                        ...
                        or utils.checkattr(previous_generator, "dg_gates")
                    )
                    else False
                )
                ...
                if conditional_gen and scenario == "task":
                    ...
                    allowed_classes = (
                        None
                        if scenario == "domain"
                        else list(range(classes_per_task * (task - 1)))
                    )
        ...
        if isinstance(model, ContinualLearner) and model.ewc_lambda > 0:
            ...
            allowed_classes = (
                ...
                if scenario == "task"
                    ...
                    list(range(classes_per_task * task))
                    if scenario == "class"
                    else None
                )
            )
        ...
        if replay_mode == "generative":
            Generative = True
            previous_generator = (
                previous_model if feedback else copy.deepcopy(generator).eval()
            )
        ...
        for i in range(args.tasks):
            print(
                " - {} {}: {:.4f}".format(
                    "For classes from task" if args.scenario == "class" else "Task",
                    i + 1,
                    accs[i],
                )
            )
        print(
            "=> Average accuracy over all {} {}: {:.4f}\n".format(
                args.tasks * classes_per_task if args.scenario == "class" else task,
                "classes" if args.scenario == "class" else "tasks",
                average_accs,
            )
        )
        if int(task) > 1:
            ...
            for i in range(int(task) - 1):
                forgetting = (
                    np.array(accs_all)[:-1, i].max() - np.array(accs_all)[-1, i]
                )
                avg_forgetting += forgetting
            avg_forgetting /= (int(task) - 1)
            print(
                f"=> Average forgetting over all {task} tasks: {avg_forgetting:.4f}\n"
            )
