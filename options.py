...
def add_task_options(parser, only_MNIST=False, single_task=False, compare_code="none", **kwargs):
    ...
    if single_task:
        task_choices = ['CIFAR10', 'CIFAR100', 'MNIST', 'MNIST28', "imagenet100"]
        task_default = 'CIFAR10'
    else:
        MNIST_tasks = ['splitMNIST', 'permMNIST']
        image_tasks = ['CIFAR100', "CIFAR10", "imagenet100"]
...
def set_defaults(args, only_MNIST=False, single_task=False, generative=True, compare_code='None', **kwargs):
    ...
    args.normalize = args.normalize if args.experiment in ('CIFAR10', 'CIFAR100', "imagenet100") else False
    args.augment = args.augment if args.experiment in ('CIFAR10', 'CIFAR100', "imagenet100") else False
    if hasattr(args, "depth"):
        args.depth = (5 if args.experiment in ('CIFAR10', 'CIFAR100', "imagenet100") else 0) if args.depth is None else args.depth
    if hasattr(args, "recon_loss"):
        args.recon_loss = (
            "MSE" if args.experiment in ('CIFAR10', 'CIFAR100', "imagenet100") else "BCE"
    ...
    if not single_task:
        ...
        args.iters = (5000 if args.experiment in {'CIFAR100', "CIFAR10", "imagenet100"} else 2000) if args.iters is None else args.iters
    ...
    if not single_task and not compare_code in ('hyper', 'bir'):
        ...
        elif args.experiment in{'CIFAR100', "CIFAR10", "imagenet100"}:
