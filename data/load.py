...
def get_multitask_experiment(
    ...
    if name == "permMNIST":
        ...
        if not only_config:
            ...
            if exception:
                ...
                permutations = [
                    np.random.permutation(config["size"] ** 2) for _ in range(tasks)
                ]
        ...
        if not only_config:
            ...
            for labels in labels_per_task:
                ...
                if not only_test:
                    train_datasets.append(
                        SubDataset(
                            mnist_train, labels, target_transform=target_transform
                        )
                    )
                test_datasets.append(
        ...
        # check for number of tasks
        if tasks > 10:
            raise ValueError("Experiment 'CIFAR10' cannot have more than 100 tasks!")
        # configurations
        ...
        if not only_config:
            ...
            for labels in labels_per_task:
                ...
                if not only_test:
                    train_datasets.append(
                        SubDataset(
                            cifar10_train, labels, target_transform=target_transform
                        )
                    )
                test_datasets.append(
    ...
    return (
        config
        if only_config
        else ((train_datasets, test_datasets), config, classes_per_task)
    )
